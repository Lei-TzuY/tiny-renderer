...
The next architectural promotion is **deterministic fragment alpha-test/discard with opacity-aware cutout shadow casters**. M30 established imported `d`/`map_d` opacity, M31 turned opacity into deterministic multisample coverage, and M32 introduced a reusable light-depth pass; the remaining cross-pass correctness gap is that geometry can still only be accepted by geometric coverage, so a mapped leaf/fence/card is treated as solid both in the camera depth/stencil path and in the shadow map. The next slice should add one explicit fragment-rejection stage and reuse it consistently in both passes rather than special-casing shadow silhouettes.

Acceptance for that slice should require:

- an explicit disabled-by-default alpha-test state with a finite normalized threshold and one documented deterministic comparison rule, propagated through direct triangles/meshes, ranges, models, prepared plans, instance batches, and heterogeneous prepared lists;
- fragment opacity continues to come from the existing uniform material `d` multiplied by optional `map_d`; no independent alpha texture channel, UV transform, sampler, or hidden material rule is introduced;
- the alpha test executes after per-sample interpolation/opacity evaluation but before stencil compare/update, depth test/write, blending, RGB ownership, and alpha-to-coverage, so a discarded sample has no framebuffer side effect;
- disabled alpha testing preserves existing byte/hash and per-sample behavior, while enabled alpha testing has deterministic threshold-boundary regressions on both 1× and 4× targets;
- alpha test and alpha-to-coverage compose explicitly: discard is a binary fragment/sample gate first, then surviving 4× samples may still be filtered by the existing deterministic alpha-to-coverage mask without changing source opacity;
- `render_directional_shadow_map` becomes material-draw aware enough to reuse the same opacity sampling and alpha-test rule for cutout casters while still using the existing raster/depth path and RGB write mask; alpha-test-disabled shadow generation remains equivalent to the M32 solid-caster behavior;
- prepared shadow-map generation retains referenced opacity textures through the already-owned prepared asset and preflights all entries/material draws before any light-depth mutation, so a malformed later cutout binding cannot partially commit earlier casters;
- deterministic regressions include a programmatic and file-driven `map_d` cutout whose camera silhouette and cast shadow agree, threshold edge cases, clipping/scissor interaction, depth/stencil immutability for discarded samples, 4× alpha-test plus alpha-to-coverage ordering, prepared/list equivalence, and fail-closed invalid state;
- the slice does not add stochastic transparency, destination alpha, sorting/OIT, PCF/soft shadows, generalized programmable fragment shaders, or performance/visual-quality claims;
- no conformance/performance claim is made without corresponding controlled evidence.

## Deliberate later work

General/full OBJ and MTL syntax, polygon triangulation, relative OBJ indices, smoothing/generated normals, multiple material libraries, general image formats, mipmaps, anisotropic filtering, sRGB handling, destination alpha, transparency sorting/OIT, programmable sample locations, sample masks, centroid interpolation, temporal antialiasing, specular/PBR lighting, percentage-closer/cascaded/soft shadows, normal maps, programmable shaders, GPU acceleration, and a general scene graph remain outside the current bounded CPU teaching architecture until a higher-value integration milestone justifies them.
