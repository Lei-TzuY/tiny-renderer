...
// work. Material draws are sorted by post-vertex-program mean view-space depth;
// farther (more-negative Z) draws execute first and equal-depth records preserve
// caller entry and canonical material-draw order. Existing blend/depth state is
// authoritative; this is not automatic transparency classification or OIT.
void draw_prepared_model_list_back_to_front(
    Framebuffer& framebuffer,
    std::span<const PreparedModelListEntry> entries,
    const Mat4& view,
    const Mat4& projection);
